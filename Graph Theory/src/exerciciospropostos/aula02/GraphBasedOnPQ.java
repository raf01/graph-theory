...
class IntegerEdge implements Comparable<IntegerEdge> {

    private Pair<Integer, Integer> edge;

    IntegerEdge(Integer v1, Integer v2) {
        edge = new Pair<Integer, Integer>(v1, v2);
    }

    public int compareTo(IntegerEdge o) {
...
class GraphBasedOnPQ {
	...
	public static void main(String[] args) {
	    ...
	    TreeSet<IntegerEdge> edges = new TreeSet<IntegerEdge>();
	    ...
	    for (int k = 0; k < vertices.length; k++) {
	      for (int klinha = 0; klinha < vertices.length; klinha++) {
	        ...
	        if ((vertices[klinha] == vertices[k] + q) || (vertices[k] % q != 0) && (vertices[klinha] == vertices[k] + 1)) {
	        	IntegerEdge newEdge = new IntegerEdge(new Integer(vertices[k]), new Integer(vertices[klinha]));
	          edges.add(newEdge);
	        }
	      }
	    }
  ...
  private static void printEdgeList(TreeSet<IntegerEdge> edges) {
